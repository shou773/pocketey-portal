...
def fetch_url(url, timeout=30):
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "PocketeyJapanEditorialBot/1.3 (+https://www.pocketey.com)",
...
def collect_feed(source):
    ...
    for node in root.findall(".//item"):
        ...
        if title and link:
            items.append({"source": source["name"], "source_type": "official_feed", "title": title, "url": link, "published": date, "summary": desc[:1200]})
    ...
    for node in root.findall(".//a:entry", ns):
        ...
        if title and link:
            items.append({"source": source["name"], "source_type": "official_feed", "title": title, "url": link, "published": date, "summary": summary[:1200]})
...
def collect_html_page(source):
    ...
    items = []
    anchor_re = re.compile(r"<a\b[^>]*?href\s*=\s*[\"']([^\"']+)[\"'][^>]*>(.*?)</a>", flags=re.I | re.S)
    for match in anchor_re.finditer(raw):
        ...
        if not relevant_link(title, context, source):
            continue
        items.append({"source": source["name"], "source_type": "official_web_page", "title": title[:300], "url": url, "published": "", "summary": context[:1400]})
...
def collect_sources(config):
    ...
    for source in config.get("official_sources", []):
        ...
        if source_type in ("rss", "atom"):
            ...
            items = []
        by_source[source.get("name", "")] = items
    ...
    for item in collected:
        ...
        if key and key not in seen:
            seen.add(key)
            unique.append(item)
    return unique, stats, by_source


def balanced_sample(by_source, limit):
    names = [name for name, items in by_source.items() if items]
...
def main():
    ...
    source_items, source_stats, by_source = collect_sources(config)
    if not source_items:
        print("No official-source items were collected. Check editorial_engine/config.json.", file=sys.stderr)
        return 3

    ai_limit = int(config.get("max_source_items_for_ai", 120))
    ai_items = balanced_sample(by_source, ai_limit)
    runtime = {"today_in_japan": today, "lookback_days": config.get("lookback_days", 7), "candidate_limit": config.get("candidate_limit", 8), "priority_topics": config.get("priority_topics", []), "source_stats": source_stats, "source_items": ai_items}
    ...
    print(f"Balanced items sent to Gemini: {len(ai_items)}")
